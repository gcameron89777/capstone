...
pdata <- cbind(target, usage_engagement_vars)
...
for ( f in folds ) {
  ...
  model <- NaiveBayes(NumChurn ~. , data=train)
...
## testing
# confusion matrix
confusionMatrix(testsetCopy$NumChurn, cv_prediction$class, positive="1")

# ROC curve
pred <- prediction(cv_prediction$posterior.1, testsetCopy$NumChurn)
